package mainwindow

import "zupper/entity"

func (w *MainWindow) UpdateStatusBar(model entity.Model) {
	if !model.Gui.MainWindow.StatusBar.Utm {
		w.SbiUtmState.SetText("УТМ")
		w.SbiUtmState.SetIcon(w.IconRed)
	} else {
		w.SbiUtmState.SetText("УТМ")
		w.SbiUtmState.SetIcon(w.IconGreen)
	}
	// if model.Gui.MainWindow.StatusBar.Scan {
	// 	w.SbiScan.SetText("Прием")
	// 	w.SbiScan.SetIcon(w.IconGreen)
	// } else {
	// 	w.SbiScan.SetText("Прием")
	// 	w.SbiScan.SetIcon(w.IconRed)
	// }
	if model.Gui.MainWindow.StatusBar.License {
		w.SbiLicense.SetText("Лицензия")
		w.SbiLicense.SetIcon(w.IconGreen)
	} else {
		w.SbiLicense.SetText("Лицензия")
		w.SbiLicense.SetIcon(w.IconRed)
	}
	w.SbiFsrarId.SetText(model.Gui.MainWindow.StatusBar.Fsrarid)
	// s := w.App.History().Last()
	// w.SbiState.SetText(s)
}
