...
var pages = map[string]types.PageConfig{
	"Home":          {Title: "Настройки", Image: "104", NewPage: newHomePage, Class: "home"},
...
func CreateTreeMenu(app types.IApp) *types.AppmenuTreeModel {
	var importMenu, trueZnakMenu, reportsMenu *types.AppMenu

	tvm := types.NewAppMenuTreeModel()
	page1 := pages["Home"]
	home := tvm.NewRootAppMenu(app, page1, page1.NewPage, imageMenu(app, page1.Image))
	...
	switch app.Configuration().Application.StartPage {
	case "home":
		tvm.SetDefaultMenu(home)
	case "import":
		if importMenu != nil {
			tvm.SetDefaultMenu(importMenu)
		} else {
			tvm.SetDefaultMenu(home)
		}
	case "4z":
		if trueZnakMenu != nil {
			tvm.SetDefaultMenu(trueZnakMenu)
		} else {
			tvm.SetDefaultMenu(home)
		}
	case "report":
		if reportsMenu != nil {
			tvm.SetDefaultMenu(reportsMenu)
		} else {
			tvm.SetDefaultMenu(home)
		}
	default:
...
func imageMenu(app IApp, img string) interface{} {
	if img == "" {
		return ""
	}
